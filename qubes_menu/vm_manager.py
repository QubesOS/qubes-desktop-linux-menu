...
class VMEntry:
    ...
    def __init__(self, vm: QubesVM):
        ...
        self._internal = bool(self.vm.features.get("internal", False))
        if not self._internal and self.parent_vm:
            self._internal = bool(
                self.parent_vm.features.get("internal", False)
            )
...
class VMManager:
    ...
    def load_vm_from_name(self, vm_name: str) -> Optional[VMEntry]:
        ...
        if vm.features.get("internal", False):
            return None
        if hasattr(vm, "template") and vm.template.features.get(
            "internal", False
        ):
            return None
